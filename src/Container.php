 ...
 * @link    www.emanuelefornasier.it
 * @version 2017-12-09
 * @license MIT License
 */

namespace SirFaenor\Container;

use SirFaenor\Container\Exceptions as Exceptions;


/**
 * Iniettore di dipendenze e contenitore di servizi.
 * 
...
class Container
{
    ...
    public function store($alias, $objectOrcallable)
    {
        ...
        if (isset($this->_services[$alias])) {
            throw new Exceptions\ServiceAlreadyRegisteredException($alias);
        }
    ...
    private function getStored($name)
    {
        ...
        if (!isset($this->_services[$name])) {
            throw new Exceptions\ServiceNotRegisteredException($name);
            return false;
        }
     ...
     * @return nuova istanza dell'alias
     */
    public function create($alias)
    {

        if(!isset($this->_factories[$alias])) {
            throw new Exceptions\ServiceNotRegisteredException($alias);
        }
    ...
    {
        throw new Exceptions\CannotModifyServiceException($k);
    }
